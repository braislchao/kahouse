...
func TestExtractOffsets(t *testing.T) {
	t.Run("empty batch", func(t *testing.T) {
		offsets, err := extractOffsets([]map[string]interface{}{})
		if err != nil {
	...
	t.Run("single record with Kafka-native types", func(t *testing.T) {
		offsets, err := extractOffsets([]map[string]interface{}{
			{"kafka_topic": "test", "kafka_partition": int32(0), "kafka_offset": kafka.Offset(100)},
		})
		if err != nil {
			t.Fatalf("Expected no error, got %v", err)
		}
		if offset, exists := offsets[0]; !exists || offset != 101 {
			t.Errorf("Expected offset 101 for partition 0, got %v (exists: %v)", offset, exists)
		}
	})

	t.Run("multiple records same partition picks max", func(t *testing.T) {
		offsets, err := extractOffsets([]map[string]interface{}{
			{"kafka_topic": "test", "kafka_partition": int64(0), "kafka_offset": int64(100)},
			{"kafka_topic": "test", "kafka_partition": int64(0), "kafka_offset": int64(105)},
			{"kafka_topic": "test", "kafka_partition": int64(0), "kafka_offset": int64(102)},
		})
		if err != nil {
			t.Fatalf("Expected no error, got %v", err)
		}
		if offset, exists := offsets[0]; !exists || offset != 106 {
			t.Errorf("Expected offset 106 for partition 0, got %v (exists: %v)", offset, exists)
		}
	})
	...
	t.Run("multiple partitions", func(t *testing.T) {
		offsets, err := extractOffsets([]map[string]interface{}{
			{"kafka_topic": "test", "kafka_partition": int64(0), "kafka_offset": int64(100)},
			{"kafka_topic": "test", "kafka_partition": int64(1), "kafka_offset": int64(200)},
			{"kafka_topic": "test", "kafka_partition": int64(0), "kafka_offset": int64(105)},
			{"kafka_topic": "test", "kafka_partition": int64(1), "kafka_offset": int64(205)},
		})
		if err != nil {
			t.Fatalf("Expected no error, got %v", err)
		}
		...
		if offset := offsets[0]; offset != 106 {
			t.Errorf("Expected offset 106 for partition 0, got %v", offset)
		}
		if offset := offsets[1]; offset != 206 {
			t.Errorf("Expected offset 206 for partition 1, got %v", offset)
		}
	})
}

func TestExtractOffsetsRejectsInvalidTypes(t *testing.T) {
	_, err := extractOffsets([]map[string]interface{}{{
		"kafka_partition": "bad",
		"kafka_offset":    int64(1),
	}})
	if err == nil {
		t.Fatal("Expected extractOffsets to reject invalid kafka_partition type")
	}
	...
	if err == nil {
		t.Fatal("Expected extractOffsets to reject invalid kafka_offset type")
	}
}
